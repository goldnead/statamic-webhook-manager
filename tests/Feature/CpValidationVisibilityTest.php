...
class CpValidationVisibilityTest extends TestCase
{
    ...
    public function test_every_setting_the_cp_validates_has_somewhere_to_show_its_error(): void
    {
        ...
        // The settings form has none: its rules are generated by walking the
        // field definition, so the largest form in the addon sat in the blind
        // spot of the very test written for this failure mode.
        //
        // The form itself now belongs to the suite's shared layer, but the
        // *fields* are still this addon's, and so is the failure: a field this
        // package offers with no rule behind it reaches `config()` unvalidated
        // and from there the delivery engine, the retry planner and the inbound
        // request path. The chain that has to hold is unchanged — the validator
        // addresses `settings.<key>`, and the screen looks up an error under
        // that same string. The dots are the interesting part:
        // `retry.max_attempts` carries real dots, and to the validator a dot is
        // a path separator, so an unescaped rule would address a nested
        // structure that does not exist and the message would arrive under a
        // key nothing on the page reads.
        $fields = app(SettingsRegistry::class)->fields(Settings::settingsNamespace());
        $this->assertNotEmpty($fields, 'The registry knows no fields — the settings form validates nothing.');

        // Built by hand rather than resolved: resolving a FormRequest through
        // the container runs its authorization, and this test is about the
        // rules. The namespace has to be on the request, because the rules are
        // derived from the section being written.
        $request = UpdateBrandSettingsRequest::create('/', 'PATCH', [
        ...
        // What this test used to do next: read
        // `vendor/goldnead/statamic-brand-context/resources/js/pages/Settings.vue`
        // and grep it for the error-bag markers. Removed on 06.09.2026, when
        // the first run against the *released* package failed on it — that
        // package marks `/resources/js` as `export-ignore`, so the file exists
        // in a dev checkout and in no installation anywhere. It also asserted
        // the wrong artefact: what a viewer renders is the built bundle under
        // `resources/dist`, not the source.
        //
        // The claim itself is still asserted, in the package that owns it:
        // `statamic-brand-context/tests/js/Settings.test.js` mounts the page
        // and checks that a field error lands on its field and that an error
        // belonging to no field is still shown. This addon's own business is
        // the half above — every field it offers has a rule behind it.
    }
    ...
    public function test_the_storage_switch_refuses_with_an_error_bag_rather_than_a_bare_status(): void
    {
        ...
        // the refusal arrives as a blank stop. The switch has one field and one
        // submission; if this regresses, its banner can never fill.
        //
        // It moved to the Debug screen with the rest of the old settings page's
        // non-settings furniture — it was never a setting, it migrates data.
        $source = file_get_contents(dirname(__DIR__, 2).'/src/Http/Controllers/Cp/DebugController.php');
